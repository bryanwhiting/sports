...
df_name = read_csv(get_path('records.csv')) %>%
  ...
  mutate(team1 = team,
         team2 = team,
         name1 = paste(name, record_result),
...
df = read_csv(get_path('all_scores.csv')) %>%
  ...
  # Replace team names
  left_join(df_name %>% select(team1, name1), on='team1') %>%
  mutate(team1 = name1) %>% select(-name1) %>%
  left_join(df_name %>% select(team2, name2), on='team2') %>%
...
sketch = htmltools::withTags(table(
  tableHeader(toupper(c('Date', 'Game', 'Elo', 'RESULT_V02'))),
