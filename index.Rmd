...
df_yesterday = df %>% 
  ...
  mutate(
    indicator1 = ifelse(pg_score1 > pg_score2, '✓', ''),
    indicator2 = ifelse(pg_score1 > pg_score2, '', '✓'),
...
dt <- df_yesterday %>%
  select(date, game, result, !!models, ends_with('_acc')) %>%
              ...
              list(
                ...
                columnDefs = list(
                  list(
                    ...
                    targets = which(colnames(.) %>% str_detect('_acc$|result')) - 1, 
                    visible = F
                  ))
              )) %>%
